...
export function LinkCard({ link, onEdit, onDelete }: LinkCardProps) {
  ...
  return (
    ...
    >
      ...
      {/* Memo indicator */}
      {link.memo && (
        <div className="mt-2 relative group/memo">
          <div className="flex items-start gap-1.5 text-xs text-muted-foreground">
            ...
            <span className="line-clamp-2 leading-relaxed">{link.memo}</span>
          </div>
          {/* Tooltip for full memo */}
          {link.memo.length > 50 && (
            <div className="absolute bottom-full left-0 right-0 mb-2 p-2 bg-popover border border-border rounded-lg shadow-lg text-xs text-popover-foreground opacity-0 group-hover/memo:opacity-100 transition-opacity pointer-events-none z-20">
              {link.memo}
            </div>
          )}
        </div>
